...
from my_dataset import ImageDataset_shear
...
def main(args):
    ...
    train_data = ImageDataset_shear(catalog=os.path.join(args.dir, 'train.ecsv'), 
                              args=args, 
    ...
    val_data = ImageDataset_shear(catalog=os.path.join(args.dir, 'validation.ecsv'), 
                            args=args, 
                            transforms=T.Compose([
                                T.KS_rec(args), 
                                T.RandomCrop(size=args.crop), 
    ...
    model = MWCNN(n_channels=in_channels, n_feats=64)
    ...
    writer = SummaryWriter('../tlogs_mw')
    # delete existing tensorboard logs
    shutil.rmtree('../tlogs_mw')
    os.mkdir('../tlogs_mw')
    ...
    for i in range(args.epochs):
        ...
        if not best_loss:
            ...
            torch.save(state_dict, f'../models/mw_e{i+1}.pth')
