...
# MWCNN total trainable params = 24928962
...
def main(args):
    ...
    model1 = MWCNN(n_channels=in_channels, n_feats=64)
    if args.load1:
        print(f'initializing model1 using {args.load1}.pth')
        state_dict = torch.load('../models/'+args.load1+'.pth', map_location=device)
        remove_prefix = 'module.'
        state_dict = {k[len(remove_prefix):] if k.startswith(remove_prefix) else k: v for k, v in state_dict.items()}
        model1.load_state_dict(state_dict)
    model2 = UNet(n_channels=in_channels)
    model = Ensemble(model1, model2, freeze1=args.freeze1)
    ...
    writer = SummaryWriter('../tlogs_esb')
    # delete existing tensorboard logs
    shutil.rmtree('../tlogs_esb')
    os.mkdir('../tlogs_esb')
    ...
    for i in range(args.epochs):
        ...
        if not best_loss:
            ...
            torch.save(state_dict, f'../models/esb_e{i+1}.pth')
...
def get_args():
    ...
    parser.add_argument("--gpu-ids", default='2', type=str, help='gpu id; multiple gpu use comma; e.g. 0,1,2')
    ...
    parser.add_argument("--load1", default=False, type=str, help='load a pre-trained .pth model for model1')
    ...
    parser.add_argument("--freeze1", default=True, action='store_false', help='freeze model1 in ensemble training; requires loading pretrained model1')
    ...
    parser.add_argument("--spac-loss", default='huber', type=str, choices=['huber', 'l1', 'ssim', 'ms-ssim', 'charbonnier'], help='spatial domain loss function')
