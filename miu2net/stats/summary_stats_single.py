...
draw6(1, true, 'True', **d)
draw6(2, ml, 'Hybrid-ML', **d)
draw6(3, ks, 'KS Deconvolution', **d)
draw6(4, wiener, 'Wiener Filtering', **d)
draw6(5, sparse, 'Sparse Recovery', **d)
...
draw6(1, gb_true, 'GB True', **d)
draw6(2, gb_ml, 'GB Hybrid-ML', **d)
draw6(3, gb_ks, 'GB KS Deconvolution', **d)
draw6(4, gb_wiener, 'GB Wiener Filtering', **d)
draw6(5, gb_sparse, 'GB Sparse Recovery', **d)
draw6(6, gb_mcalens, 'GB MCALens', **d)
cbar(fig, cax=plt.axes([0.88, 0.08, 0.04, 0.8]))
plt.show()
...
d = dict(scale=[ml.min(), ml.max()], cmap='viridis')
draw4(1, gb_ml, f'Blurred Prediction ({len(pcoord)} peaks)', **d)
plt.scatter(pcoord[:, 1], pcoord[:, 0], s=30, color='yellow', marker='+', linewidth=0.8)
draw4(2, gb_true, f'Blurred True ({len(tcoord)} peaks)', **d)
plt.scatter(tcoord[:, 1], tcoord[:, 0], s=30, color='yellow', marker='+', linewidth=0.8)
draw4(3, gb_ml, f'Blurred Prediction ({len(matched_pcoord)} matched peaks)', **d)
plt.scatter(matched_pcoord[:, 1], matched_pcoord[:, 0], s=30, color='yellow', marker='+', linewidth=0.8)
draw4(4, gb_true, f'Blurred True ({len(sub_tcoord)} peaks)', **d)
